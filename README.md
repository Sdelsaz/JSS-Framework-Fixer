...
A script to help re-deploy the Jamf Pro Framework to a Smart Computer Group. An existing Smart Computer Group can be can used or a new one can be created. This script leverages the Jamf Pro API and is to be run on a mac, it is not meant to be deployed using Jamf Pro.  Prompts are used to gather the server details credentials for the API calls.
