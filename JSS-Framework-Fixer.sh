...
icon="https://github.com/Sdelsaz/Framework-Fixer/blob/main/Images/Framework-Fixer-icon.png?raw=true"

#######################################################################################################
# Check if swiftDialog is installed. if not, inform the user of the pre-requirements
#######################################################################################################
echo "######################################START LOGGING#############################################
$(
...
credential_prompt() {
  ...
  if [ $? == 0 ]
  then
    jamf_pro_url=$(echo "${server_details}" | /usr/bin/plutil -extract "Jamf Pro URL" xml1 -o - - | xmllint --xpath "string(//string)" -)
    username=$(echo "${server_details}" | /usr/bin/plutil -extract "${username_label}" xml1 -o - - | xmllint --xpath "string(//string)" -)
    api_password=$(echo "${server_details}" | /usr/bin/plutil -extract "${password_label}" xml1 -o - - | xmllint --xpath "string(//string)" -)
  else
    echo "$(date '+%Y-%m-%d %H:%M:%S') WARNING: User Cancelled" >> ${log_file}
    exit 0
  fi
...
group_name_prompt() {
  ...
  if [ $? == 0 ]
  then
    group_name=$(echo "${group_name}" | awk -F '"' '{print $4}' | tr -d '\r\n' | sed 's/[^[:print:]]//g')
  else
    echo "$(date '+%Y-%m-%d %H:%M:%S') WARNING: User Cancelled" >> ${log_file}
    invalidate_token
    exit 0
  ...
  echo "$(date '+%Y-%m-%d %H:%M:%S') INFO: Checking if a group called ${group_name} exists" >> ${log_file}
  ...
  echo "$(date '+%Y-%m-%d %H:%M:%S') ERROR: Could not check if a group called ${group_name} exists" >> ${log_file}
  error_prompt
  invalidate_token
  exit 1
  fi
  # Check if we got any results from the check / Check to make sure a group with the provided name exists
  count=$(printf '%s\n' "$group_check" | tr -d '\n' | sed -n 's/.*"totalCount"[[:space:]]*:[[:space:]]*\([0-9][0-9]*\).*/\1/p')
  # If not, present an error message  
  if [[ "$count" -eq 0 ]]; then
    echo "$(date '+%Y-%m-%d %H:%M:%S') ERROR: Could not find a Smart Computer Group named ${group_name}" >> "$log_file"
    group_not_found
  fi
}
...
group_not_found() {
  ${dialog_path} \
    --title "Framework Fixer" \
    --message "Oops! We could not find a Smart Computer Group called ${group_name}" \
    --icon "${icon}" \
    --overlayicon "caution" \
    --alignment "left" \
    --small \
...
check_token_expiration_prompt() {
  ...
  then
    echo "$(date '+%Y-%m-%d %H:%M:%S') INFO: Token valid until the following epoch time: " "${token_expiration_epoch}epoch" >> ${log_file}
  else
...
# Prompt to choose new or existing group
group_option_prompt
echo "$(date '+%Y-%m-%d %H:%M:%S') INFO: Prompting to choose between new group of existing group" >> ${log_file}

# Check if a Smart Computer Group already exists
if [[ "${group_selection}" == "I already have a Smart Computer Group" ]]; then

  # Request the name of the existing Smart Computer Group
  echo "$(date '+%Y-%m-%d %H:%M:%S') INFO: Existing group workflow selected.  Prompting for existing group name" >> ${log_file}
  group_name_prompt
...
if [[ "${group_selection}" == "Please create a Smart Computer Group" ]]; then
  ...
  # Request the name of the Smart Computer Group to be created
  echo "$(date '+%Y-%m-%d %H:%M:%S') INFO: Prompting for new group name" >> ${log_file}
  new_group_prompt
...
EOM
  ...
  group_creation=$(check_response -X 'POST' \
    "${jamf_pro_url}/api/v2/computer-groups/smart-groups" -H "accept: application/json" -H "Authorization: Bearer ${bearer_token}" -H "Content-Type: application/json" -d "${json_payload}" -b "${ap_balance_id}")
